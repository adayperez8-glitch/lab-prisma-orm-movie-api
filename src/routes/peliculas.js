...
// POST /api/peliculas (transacción con upsert)
...
router.get('/:id/resenas', verificarToken, async (req, res, next) => {
  try {
    const peliculaId = Number(req.params.id)
    await prisma.pelicula.findUniqueOrThrow({ where: { id: peliculaId } })
...
router.post('/:id/resenas', verificarToken, async (req, res, next) => {
  try {
    const peliculaId = Number(req.params.id)
...
// FAVORITOS DENTRO DE PELICULAS
const { añadirFavorito, quitarFavorito, listarFavoritos } = require('../controllers/favoritosController')

router.post('/:peliculaId/favoritos', verificarToken, añadirFavorito)
router.delete('/:peliculaId/favoritos', verificarToken, quitarFavorito)
